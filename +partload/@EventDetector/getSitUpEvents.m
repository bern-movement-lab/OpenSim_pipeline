function timeRange = getSitUpEvents(obj)
%GETSITUPEVENTS Get start and endtime for sitUp events from kinematic data.
%
%   This method gets the start and end time for the task "sitUp" by
%   analysing the trajectory of the SACR (sacrum) marker.
%   The start time is defined as the timepoint when the forward sagital velocity
%   of the SACR marker exceeds 5% of the overall maximal SACR forward
%   sagital velocity for the first time.
%   The end time is defined as the timepoint when the upward vertical
%   velocity of the SACR marker falls below the 5% of the maximal vertical
...
% get event start depending on threshold sagital velocity
maxSagitalVelocity = max(vStrnX);
thresholdSagitalVelocity = threshold * maxSagitalVelocity;

startIX = find(vStrnX>thresholdSagitalVelocity,1);
if isempty(startIX)
    startIX = 1;
end

% get event end depending on threshold vertical velocity
[maxVerticalVelocity, maxVerticalVelocityIX] = max(vStrnY);
thresholdVerticalVelocity = threshold * maxVerticalVelocity;

endIX = find(time>time(maxVerticalVelocityIX) & vStrnY<thresholdVerticalVelocity,1);
if isempty(endIX)
    endIX = numel(time);
else
    endIX = endIX-1;
